...
module.exports = {
  'request': {
    'param': ->
      ...
      uri = 'foo/bar/baz'
      request = Request.factory(path: uri)

      request.execute([route])

      expect(request.param('id')).to.be('baz')
      expect(request.params).to.not.have.property('foo')
      expect(request.path).to.equal(uri)

      expect(request.params).to.not.have.property('controller')
      expect(request.params).to.not.have.property('action')
      expect(request.params).to.not.have.property('directory')
      expect(request.params).to.not.have.property('uri')
      expect(request.params).to.not.have.property('route')

      route = Route('(<uri>)', 'uri': '.+')
      route.defaults('controller': 'foobar', 'action': 'index')
      request = Request.factory(path: 'foobar')

      request.execute([route])

      expect(request.param('uri')).to.equal('foobar')
  }
}
