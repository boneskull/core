module.exports = {
  ...
  wrapConditionalPromise: function(context, funcName, isReturn, baseContext) {
    var Q, defined, original;
    if (isReturn == null) {
      isReturn = false;
    }
    if (baseContext == null) {
    ...
    return context[funcName] = function() {
      ...
      if (args.length === defined) {
        return original.apply(baseContext || context, args);
      } else {
        d = Q.defer();
  ...
  noop: function() {},
  assignObject: function(out, path, value) {
    if (this.isArray(path)) {
      path = path.join('.');
    }
    return this.$.g.set(out, path, value);
  }
};
