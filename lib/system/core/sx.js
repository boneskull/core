...
module.exports = ES5Class.define('sx', {}, function() {
  var classes, configs, dependencyLoader, init, setPaths, sxChannel,
    _this = this;
  sxChannel = postal.channel('sx');
  init = function() {
    var loaddir;
    return loaddir = function(spath, ext) {
      if (ext == null) {
        ext = '';
      }
  ...
  dependencyLoader = function(data) {
    ...
    if (data["class"] != null) {
      ...
      if ((data["class"].$initialize != null) && (data["class"].$singleton == null)) {
  ...
  return {
    ...
    load: function(name, where) {
      ...
      if (_path && fs.existsSync(_path)) {
        declaration = require(_path);
        cls = this.factory(name, declaration);
        return cls;
      }
      return false;
    }
